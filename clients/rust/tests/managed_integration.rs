...
fn managed_lifecycle_integration() {
    ...
    client.put("managed-rust", b"value", None).expect("managed put");
    assert_eq!(client.get("managed-rust").expect("managed get"), Some(b"value".to_vec()));
    drop(client);
    ...
    let mut tcp = Client::connect_managed(ManagedOptions {
        ...
        transport: ManagedTransport::Tcp { host: Ipv4Addr::LOCALHOST, port },
        idle_timeout: Duration::from_millis(250),
        startup_timeout: Duration::from_secs(5),
        auth_token: None,
    }).expect("managed TCP connect");
    tcp.put("managed-rust-tcp", b"value", None).expect("managed TCP put");
    assert_eq!(tcp.get("managed-rust-tcp").expect("managed TCP get"), Some(b"value".to_vec()));
    drop(tcp);
