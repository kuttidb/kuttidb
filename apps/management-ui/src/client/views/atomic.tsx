...
import { Atom, Braces, CheckCircle2, History } from "lucide-react";
...
import { JobCompletionView, CompletionHistoryView } from "@/views/job-completion";
...
export function AtomicView({ profileId, onOpenQueues }: { profileId: string; onOpenQueues?: () => void }) {
  const [tab, setTab] = useState<string>("operations");
  return (
    <div>
      <PageHeader
        title="Atomic operations"
        description="All-or-nothing commits: keyspace mutations with queue/routing delivery, and atomic job completion for durable queues."
      />
      <Tabs value={tab} onValueChange={setTab}>
        <TabsList className="mb-4">
          <TabsTrigger value="operations">Atomic operations</TabsTrigger>
          <TabsTrigger value="completion"><CheckCircle2 className="size-3.5 mr-1" />Complete Queue job</TabsTrigger>
          <TabsTrigger value="receipts"><History className="size-3.5 mr-1" />Completion receipts</TabsTrigger>
        </TabsList>
        <TabsContent value="operations"><AtomicOperationsForm profileId={profileId} /></TabsContent>
        <TabsContent value="completion"><JobCompletionView profileId={profileId} {...(onOpenQueues ? { onOpenQueues } : {})} /></TabsContent>
        <TabsContent value="receipts"><CompletionHistoryView profileId={profileId} /></TabsContent>
      </Tabs>
    </div>
  );
}

function AtomicOperationsForm({ profileId }: { profileId: string }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      // intent — retries are never automatic. This legacy form keeps its own
      // non-idempotent semantics; atomic job completion never inherits them.
    }
    ...
    <div className="grid grid-cols-1 items-start gap-6 lg:grid-cols-[3fr_2fr] lg:gap-8">
      ...
      <div className="grid gap-6">
        <Section title="Transaction preview">
          ...
          <p className="mt-3 text-xs text-muted-foreground">Both sides commit under one durable transaction ID: after recovery, both exist or neither does. Each commit sends its own idempotency key, so a repeated intent is detected server-side. This legacy form never auto-retries; atomic job completion keeps its own idempotent retry rules under Complete Queue job.</p>
        </Section>
        {result && (
